...
def _build_cfg(params: dict[str, Any]) -> tuple[MethodConfig, str, int, int, int, float]:
    ...
    cfg = MethodConfig(
        ...
        corr_divide_denom=bool(params.get("corr_divide_denom", True)),
        corr_water_level=float(params.get("corr_water_level", 1e-4)),
        corr_shift_sec=float(params.get("corr_shift_sec", 0.0)),
